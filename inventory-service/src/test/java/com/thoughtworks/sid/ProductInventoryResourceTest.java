...
public class ProductInventoryResourceTest {
    ...
    final String INVENTORY_URL = "/api/stores/" + STORE_ID + "/products/" + PRODUCT_ID;
    ...
    public void should_get_401_to_get_inventories() throws Exception {
        RequestBuilder requestBuilder = get(INVENTORY_URL + "/inventories");
    ...
    public void should_get_200_to_get_stores_with_principal() throws Exception {
        ...
        RequestBuilder requestBuilder = get(INVENTORY_URL + "/inventories").principal(principal);
        mvc.perform(requestBuilder)
    ...
    public void should_success_to_create_inventory() throws Exception {
        ...
        RequestBuilder requestBuilder = post(INVENTORY_URL + "/inventories").principal(principal)
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .content(objectMapper.writeValueAsString(INVENTORY));
        mvc.perform(requestBuilder)
                .andExpect(status().isCreated());

        requestBuilder = get(INVENTORY_URL + "/inventories").principal(principal);
        mvc.perform(requestBuilder)
                .andExpect(status().isOk())
    ...
    public void should_fail_to_find_inexist_inventory() throws Exception {
        RequestBuilder requestBuilder = get(INVENTORY_URL + "/inventories/" +INVENTORY_ID).principal(principal);
        mvc.perform(requestBuilder)
